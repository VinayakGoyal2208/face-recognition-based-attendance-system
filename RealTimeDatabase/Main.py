    ...
    if faceCurFrame:
        ...
        if counter != 0:
            if counter == 1:
                ...
                if blob is not None:  # Check if blob exists
                    array = np.frombuffer(blob.download_as_string(), np.uint8)
                    imgStudent = cv2.imdecode(array, cv2.IMREAD_COLOR)  # Use cv2.IMREAD_COLOR flag
                    ...
                    modeType = 3  # Change modeType to handle no image found scenario
                    counter = 0
                    imgBackground[44:44 + 633, 808:808 + 414] = imgModeList[modeType]
